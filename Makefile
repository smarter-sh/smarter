...
docker-init:
	...
	docker exec smarter-app bash -c "\
		...
		python manage.py create_sqldb_connection --host sql.lawrencemcdaniel.com --db smarter_test_db --user smarter_test_user \
...
docker-test:
	make docker-check && \
	docker exec smarter-app bash -c "./manage.py test smarter.apps.chatbot.tests.test_managepy_commands"
