...
/**
 * Vite Orchestrator: addCustomManifestData
 *
 * This orchestrator injects custom metadata into the generated manifest.json file after each build.
...
/**
 * Vite Orchestrator: postBuildOrchestrator
 *
 * After each build, this orchestrator optionally uploads the built assets to S3 and triggers a CloudFront invalidation,
 ...
 */
const postBuildOrchestrator: PluginOption = {
  name: "post-build",
 ...
 * settings based on the build command (development or production). It sets up orchestrators, build output,
 * asset handling, and development server proxying to integrate seamlessly with the Django backend.
 *
 * Key features:
 * - Uses custom orchestrators for manifest metadata and optional CDN deployment
...
export default defineConfig(({ command }: ConfigEnv) => ({
  orchestrators: [react(), postBuildOrchestrator, addCustomManifestData],
  ...
  server: {
    proxy: {
      ...
      "/orchestrator/": "http://localhost:9357",
    },
  },
}));
