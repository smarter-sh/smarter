...
/**
 * Vite MCPClient: addCustomManifestData
 *
 * This mcpclient injects custom metadata into the generated manifest.json file after each build.
...
/**
 * Vite MCPClient: postBuildMCPClient
 *
 * After each build, this mcpclient optionally uploads the built assets to S3 and triggers a CloudFront invalidation,
 ...
 */
const postBuildMCPClient: PluginOption = {
  name: "post-build",
 ...
 * settings based on the build command (development or production). It sets up mcpclients, build output,
 * asset handling, and development server proxying to integrate seamlessly with the Django backend.
 *
 * Key features:
 * - Uses custom mcpclients for manifest metadata and optional CDN deployment
...
export default defineConfig(({ command }: ConfigEnv) => ({
  mcpclients: [react(), postBuildMCPClient, addCustomManifestData],
