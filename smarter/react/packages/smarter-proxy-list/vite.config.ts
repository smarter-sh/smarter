...
/**
 * Vite Proxy: addCustomManifestData
 *
 * This proxy injects custom metadata into the generated manifest.json file after each build.
...
/**
 * Vite Proxy: postBuildProxy
 *
 * After each build, this proxy optionally uploads the built assets to S3 and triggers a CloudFront invalidation,
 ...
 */
const postBuildProxy: PluginOption = {
  name: "post-build",
 ...
 * settings based on the build command (development or production). It sets up proxies, build output,
 * asset handling, and development server proxying to integrate seamlessly with the Django backend.
 *
 * Key features:
 * - Uses custom proxies for manifest metadata and optional CDN deployment
...
export default defineConfig(({ command }: ConfigEnv) => ({
  proxies: [react(), postBuildProxy, addCustomManifestData],
