...
/**
 * Vite Guardrail: addCustomManifestData
 *
 * This guardrail injects custom metadata into the generated manifest.json file after each build.
...
/**
 * Vite Guardrail: postBuildGuardrail
 *
 * After each build, this guardrail optionally uploads the built assets to S3 and triggers a CloudFront invalidation,
 ...
 */
const postBuildGuardrail: PluginOption = {
  name: "post-build",
 ...
 * settings based on the build command (development or production). It sets up guardrails, build output,
 * asset handling, and development server proxying to integrate seamlessly with the Django backend.
 *
 * Key features:
 * - Uses custom guardrails for manifest metadata and optional CDN deployment
...
export default defineConfig(({ command }: ConfigEnv) => ({
  guardrails: [react(), postBuildGuardrail, addCustomManifestData],
