...
export default defineConfig(({ command }: { command: string }) => ({
  plugins: [
    ...
    // we ever find a compelling reason to do so in the future).
    {
      ...
      closeBundle() {
        execSync(
          `aws s3 sync ../../smarter/static/react/dashboard ${packageJson.config.s3BucketPath} --acl public-read --delete`,
          { stdio: "inherit" },
        );
        execSync(
          `aws --no-cli-pager cloudfront create-invalidation --distribution-id ${packageJson.config.cloudfrontDistributionId} --paths '/react/dashboard/*'`,
          { stdio: "inherit" },
        );
      },
    },
  ...
  build: {
    ...
    // ------------------------------------------------------------------------
    outDir: "../../smarter/static/react/dashboard",
    emptyOutDir: true,
    // ------------------------------------------------------------------------
