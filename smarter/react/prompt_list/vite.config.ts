...
export default defineConfig(({ command }: { command: string }) => ({
  plugins: [
    ...
    // we ever find a compelling reason to do so in the future).
    {
      ...
      closeBundle() {
        execSync(
          `aws s3 sync ../../smarter/static/react/prompt_list ${packageJson.config.s3BucketPath} --acl public-read --delete`,
          { stdio: "inherit" },
        );
        execSync(
          `aws --no-cli-pager cloudfront create-invalidation --distribution-id ${packageJson.config.cloudfrontDistributionId} --paths '/react/prompt_list/*'`,
          { stdio: "inherit" },
        );
      },
    },
  ...
  base: command === "serve" ? "/" : "/static/react/prompt_list/",
  resolve: {
    alias: {
  ...
  build: {
    ...
    // ------------------------------------------------------------------------
    outDir: "../../smarter/static/react/prompt_list",
    emptyOutDir: true,
    // ------------------------------------------------------------------------
