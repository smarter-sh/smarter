"""Test VectorsearchApiBaseViewSet."""
...
from ..base import VectorsearchApiBaseViewSet
...
class TestVectorsearchApiBaseViewSet(TestAccountMixin):
    """Test SAM Vectorsearch Broker."""
    ...
    def setUpClass(cls):
        ...
        super().setUpClass()
        config_path = os.path.join(HERE, "data/vectorsearch.yaml")
        ...
        # name: test_vectorsearch
        cls.broker = SAMVectorsearchBroker(
            request=cls.request,
            account=cls.account,
            loader=cls.loader,
        )
    ...
    def test_base_class_properties(self):
        base_class = VectorsearchApiBaseViewSet(request=self.request)

        # invoke dispatch method in order to set our class properties
        base_class.dispatch(request=self.request, name=self.broker.vectorsearch.name)  # type: ignore

        logger.debug("test_base_class_properties() request=%s name=%s", self.request, self.broker.vectorsearch.name)  # type: ignore
