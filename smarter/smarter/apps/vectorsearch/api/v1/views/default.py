...
class DefaultVectorsearchApiView(VectorsearchApiBaseViewSet):
    ...
    def dispatch(self, request, *args, **kwargs):
        """
        Smarter API Vectorsearch dispatch method.

        :param request: Django HttpRequest object
        :param args: Additional positional arguments
        :param name: Vectorsearch name (str, optional)
        ...
        **Example request payload**:
        ...
        if hashed_id:
            self._vectorsearch_id = Vectorsearch.id_from_hashed_id(hashed_id)
        else:
            self._vectorsearch_id = kwargs.pop("vectorsearch_id", None)
        self._name = kwargs.pop("name", None)
        logger.info("%s - dispatch() %s %s ", self.formatted_class_name, self.vectorsearch, self.user_profile)
        ...
        # pylint: disable=broad-except
            ...
            logger.error("DefaultVectorsearchApiView.dispatch: %s, %s", e, err_traceback)
            retval = JsonResponse(
