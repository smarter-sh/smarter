"""
Vite manifest loader and asset collector for Django template Prompt List.

This module provides template tags and utility functions to load and process
the Vite manifest.json for the prompt list frontend, enabling Django
...
- prompt_list_vite_assets(entry="index.html"): Django template tag that returns the JS and CSS assets for a given Vite entry.

Example
-------
In a Django template, use the provided tag to get asset paths::

    {% load vite_prompt_list %}
    {% prompt_list_vite_assets "index.html" as assets %}
...
def load_manifest() -> dict[str, Any]:
    ...
    :rtype: dict[str, Any]
    """
    url = os.path.join(smarter_settings.root_cdn_url, "react/prompt_list/manifest.json")
...
@register.simple_tag
@cache_results()
def prompt_list_vite_assets(entry: str = "index.html") -> dict[str, Any]:
    """
