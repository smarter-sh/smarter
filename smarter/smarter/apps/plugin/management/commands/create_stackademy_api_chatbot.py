...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.NOTICE("smarter.apps.plugin.management.commands.create_stackademy_api_chatbot started.")
        )
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR("smarter.apps.plugin.management.commands.create_stackademy_api_chatbot failed.")
            )
            raise SmarterValueError(f"Failed to apply manifest: {exc}") from exc

        self.stdout.write(
            self.style.SUCCESS("smarter.apps.plugin.management.commands.create_stackademy_api_chatbot completed.")
        )
