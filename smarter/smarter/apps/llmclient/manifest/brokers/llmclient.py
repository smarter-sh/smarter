...
from smarter.apps.llmclient.manifest.models.llmclient.spec import (
    SAMLLMClientSpec,
    SAMLLMClientSpecConfig,
)
from smarter.apps.llmclient.manifest.models.llmclient.status import SAMLLMClientStatus
...
class SAMLLMClientBroker(AbstractBroker):
    ...
    This class provides a high-level abstraction for managing llmclient manifests
    within the Smarter platform. It acts as the central coordinator for the
    lifecycle of llmclient manifests, bridging the gap between declarative YAML
    files and persistent application state.

    The broker is responsible for:

    - Managing the lifecycle of llmclient manifests, including loading, validation,
      and parsing of YAML files.
    - Initializing Pydantic models from manifest data to ensure robust schema
      validation and serialization.
    - Integrating with Django ORM models that represent llmclient manifests,
    ...
    - Coordinating composite models, such as LLMClient, LLMClientAPIKey,
      LLMClientPlugin, and LLMClientFunctions, to ensure all components of an llmclient
    ...
    lifecycle management of llmclients in the Smarter Framework.
    """
    ...
    _llmclient_api_key: Optional[LLMClientAPIKey] = None
    ...
    def llmclient(self) -> Optional[LLMClient]:
        ...
        model stores the configuration and runtime state of the llmclient, and is used for all database
        operations related to the llmclient's lifecycle.
        ...
        .. note::
            ...
            plugins, and functions, and for performing updates or queries on the llmclient's state.
        ...
        try:
            self._llmclient = LLMClient.get_cached_object(
                invalidate=True, user_profile=self.user_profile, name=self.name
            )
            logger.debug(
                "%s.llmclient() retrieved existing LLMClient instance %s owned by %s from database.",
                self.formatted_class_name,
                self._llmclient,
                self.user_profile,
            )
        ...
        logger.debug(
            "%s.llmclient() LLMClient instance not found for user_profile %s. Attempting to create a new instance.",
            self.formatted_class_name,
            self.user_profile,
        )
        if self.manifest:
            ...
            logger.debug("%s.llmclient() Creating new LLMClient with data: %s", self.formatted_class_name, data)
            tags = data.pop("tags", [])
            self._llmclient = LLMClient.objects.create(**data)
            if self._llmclient and tags:
                self._llmclient.tags.set(tags)
            self._created = True
            logger.warning(
                "%s.llmclient() lazily created new LLMClient instance %s owned by %s. This logic should be handled in apply().",
                self.formatted_class_name,
                self._llmclient,
                self.user_profile,
            )
            ...
            logger.warning(
                "%s.llmclient() %s not found for user_profile %s",
                self.formatted_class_name,
                self._llmclient,
                self.user_profile,
            )
    ...
    def functions(self) -> Optional[List[str]]:
        ...
        queryset = LLMClientFunctions.objects.filter(llmclient=self.llmclient)
    ...
    def plugins(self) -> Optional[List[str]]:
        ...
        queryset = LLMClientPlugin.objects.filter(llmclient=self.llmclient)
        self._plugins = list(queryset.values_list("plugin_meta__name", flat=True))

        return self._plugins

    @property
    def llmclient_api_key(self) -> Optional[LLMClientAPIKey]:
        """
        ...
        """
        if self._llmclient_api_key:
            return self._llmclient_api_key
        try:
            self._llmclient_api_key = LLMClientAPIKey.objects.get(llmclient=self.llmclient)
    ...
    def django_orm_to_manifest_dict(self) -> Optional[dict]:
        ...
        necessary to reconstruct the manifest for the llmclient. This enables seamless round-trip conversion between
        ...
        - :py:meth:`smarter.apps.llmclient.manifest.brokers.llmclient.SAMLLMClientBroker.manifest_to_django_orm`
        - :py:class:`smarter.apps.llmclient.manifest.models.llmclient.SAMLLMClient`
        - :py:class:`smarter.apps.llmclient.manifest.models.llmclient.metadata.SAMLLMClientMetadata`
        - :py:class:`smarter.apps.llmclient.manifest.models.llmclient.spec.SAMLLMClientSpec`
        - :py:class:`smarter.apps.llmclient.manifest.models.llmclient.status.SAMLLMClientStatus`
        """
        if not self.account:
            raise SAMBrokerErrorNotReady(
        ...
        if not self.llmclient:
            ...
            return None
        llmclient_dict = model_to_dict(self.llmclient)
        llmclient_dict = self.to_camel_case(llmclient_dict)
        if not isinstance(llmclient_dict, dict):
            raise SAMLLMClientBrokerError(
                f"Failed to convert {self.kind} {self.llmclient.name} to dict", thing=self.kind
            )
        llmclient_dict.pop("id")
        llmclient_dict.pop("name")
        llmclient_dict.pop("description")
        llmclient_dict.pop("version")

        plugins = LLMClientPlugin.objects.filter(llmclient=self.llmclient)
        ...
        meta = SAMLLMClientMetadata(
            name=self.llmclient.name,
            description=self.llmclient.description,
            version=self.llmclient.version,
            tags=self.llmclient.tags_list,
        ...
        status = SAMLLMClientStatus(
            ...
            recordLocator=self.llmclient.record_locator,
            created=self.llmclient.created_at,
            modified=self.llmclient.updated_at,
            deployed=self.llmclient.deployed,
            defaultHost=self.llmclient.default_host,
            sandboxHost=self.llmclient.sandbox_host,
            hostname=self.llmclient.hostname,
            dnsVerificationStatus=self.llmclient.dns_verification_status,
            customUrl=self.llmclient.custom_url,
            defaultUrl=self.llmclient.default_url,
            sandboxUrl=self.llmclient.sandbox_url,
            url=self.llmclient.url,
            urlLLMClient=self.llmclient.url_llmclient,
            urlChatConfig=self.llmclient.url_chat_config,
            urlChatapp=self.llmclient.url_chatapp,
        )
        model = SAMLLMClient(
    ...
    def kind(self) -> str:
        ...
        allowing the system to recognize and process llmclient manifests appropriately. The kind value is defined as a constant in the llmclient manifest model
        and is used throughout the broker to ensure consistency when handling llmclient manifests.
    ...
    def manifest(self) -> Optional[SAMLLMClient]:
        ...
        an llmclient within the Smarter platform.
        ...
        :returns: An instance of ``SAMLLMClient`` representing the llmclient manifest, or ``None``
    ...
    def cache_invalidations(self) -> None:
        ...
        # -----------------------------
        LLMClient.get_cached_object(pk=self.llmclient.id, invalidate=True)  # type: ignore

        # 2.) invalidate anything else in which the llmclient is part of. this could
        ...
        # -----------------------------
        llmclient_functions = LLMClientFunctions.objects.filter(llmclient=self.llmclient)
        for llmclient_function in llmclient_functions:
            LLMClientFunctions.get_cached_object(pk=llmclient_function.id, invalidate=True)  # type: ignore

        llmclient_plugins = LLMClientPlugin.objects.filter(llmclient=self.llmclient)
        for llmclient_plugin in llmclient_plugins:
            LLMClientPlugin.get_cached_object(pk=llmclient_plugin.id, invalidate=True)  # type: ignore

        llmclient_api_keys = LLMClientAPIKey.objects.filter(llmclient=self.llmclient)
        for llmclient_api_key in llmclient_api_keys:
            LLMClientAPIKey.get_cached_object(pk=llmclient_api_key.id, invalidate=True)  # type: ignore
    ...
    def example_manifest(self, request: HttpRequest, *args, **kwargs) -> SmarterJournaledJsonResponse:
        ...
        - :py:class:`smarter.apps.llmclient.manifest.enum.SAMMetadataKeys`
        - :py:class:`smarter.apps.llmclient.manifest.enum.SCLIResponseGet`
        - :py:class:`smarter.apps.llmclient.manifest.enum.SCLIResponseGetData`
        ...
        meta_data = SAMLLMClientMetadata(
            ...
            description="This is an example llmclient manifest generated by the SAMLLMClientBroker. It serves as a template for creating your own llmclient manifests.",
            version="1.0.0",
        ...
        status = SAMLLMClientStatus(
            ...
            deployed=False,
            defaultHost="example-llmclient.smarterapi.com",
            defaultUrl="https://example-llmclient.smarterapi.com",
            customUrl=None,
            sandboxHost="https://example.com",
            sandboxUrl="https://example.com/api/v1/llm-clients/1/",
            hostname="example-llmclient.smarterapi.com",
            url="https://example.com/api/v1/llm-clients/1/",
            urlLLMClient="https://example-llmclient.smarterapi.com/llmclient",
            urlChatapp="https://example-llmclient.smarterapi.com/chatapp",
            urlChatConfig="https://example-llmclient.smarterapi.com/chatconfig",
    ...
    def get(self, request: HttpRequest, *args, **kwargs) -> SmarterJournaledJsonResponse:
        ...
        if name:
            llmclients = LLMClient.objects.filter(user_profile__account=self.account, name=name)
        else:
            llmclients = LLMClient.objects.filter(user_profile__account=self.account)
        valid_owners = valid_resource_owners_for_user(user_profile=self.user_profile)
        llmclients = llmclients.filter(user_profile__in=valid_owners).order_by("name")[:MAX_RESULTS]
        logger.debug(
            "%s.get() found %s LLMClients for account %s", self.formatted_class_name, llmclients.count(), self.account
        )

        # iterate over the QuerySet and use a serializer to create a model dump for each LLMClient
        for llmclient in llmclients:
            try:
                model_dump = LLMClientSerializer(llmclient).data
                if not model_dump:
                    raise SAMLLMClientBrokerError(
                        f"Model dump failed for {self.kind} {llmclient.name}", thing=self.kind, command=command
                    )
                ...
                raise SAMLLMClientBrokerError(
                    f"Failed to serialize {self.kind} {llmclient.name}", thing=self.kind, command=command
                ) from e
        data = {
    ...
    def apply(self, request: HttpRequest, *args, **kwargs) -> SmarterJournaledJsonResponse:
        ...
        with transaction.atomic():
            ...
            try:
                ...
                try:
                    if not isinstance(self.llmclient.tags, TaggableManager):
                        logger.warning(
                            "%s.apply() llmclient.tags is a list instead of a TaggableManager for %s %s owned by %s. This is unexpected and may indicate an issue with the LLMClient model definition or the database state. Tags=%s",
                            self.formatted_class_name,
                            self.kind,
            ...
            if self.manifest.spec.apiKey:
                ...
                for key in LLMClientAPIKey.objects.filter(llmclient=self.llmclient):
                    if key.api_key != api_key:
                        key.delete()
                        logger.debug("%s.apply() Detached SmarterAuthToken %s from LLMClient %s", self.formatted_class_name, key.name, self.llmclient.name)  # type: ignore[union-attr]
                _, created = LLMClientAPIKey.objects.get_or_create(llmclient=self.llmclient, api_key=api_key)
                if created:
                    logger.debug(
            ...
            # -------------
            for plugin in LLMClientPlugin.objects.filter(llmclient=self.llmclient):
            ...
            if self.manifest.spec.plugins:
                for plugin_name in self.manifest.spec.plugins:
                    ...
                    _, created = LLMClientPlugin.objects.get_or_create(llmclient=self.llmclient, plugin_meta=plugin)
                    if created:
                        logger.debug(
            ...
            # -------------
            for function in LLMClientFunctions.objects.filter(llmclient=self.llmclient):
