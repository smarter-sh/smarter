...
class LLMClient(MetaDataWithOwnershipModel):
    ...
    This Django model represents an llmclient instance associated with a specific customer account.
    It provides configuration, deployment status, domain management, and API endpoint properties
    for each llmclient. The model supports multiple modes of operation (sandbox, custom, default),
    DNS verification, TLS certificate management, and integration with external providers.

    **Key Features**

    - Associates each llmclient with a customer :class:`Account`.
    ...
    - Serializes llmclient configuration for API and frontend consumption.

    **Usage Example**

    .. code-block:: python

        llmclient = LLMClient.objects.get(account=my_account, name="example")
        if llmclient.ready():
            print(llmclient.url_llmclient)
    ...
    **See Also**
    ...
    #: Example: LLMClientCustomDomainDNS(id=1, domain="my-llmclient.example.com")
    ...
    app_name = models.CharField(default="llmclient", max_length=255, blank=True, null=True)
    ...
    app_welcome_message = models.CharField(default="Welcome to the llmclient!", max_length=255, blank=True, null=True)
    ...
    def sandbox_url(self) -> str:
        ...
        being used in the Smarter Workbench. maps to "<int:llmclient_id>/"
    ...
    def url(self) -> str:
        ...
        example: 'https://my-llmclient.1234-5678-9012.alpha.api.example.com' (custom, deployed)
    ...
    def url_llmclient(self) -> str:
        ...
        :returns: URL for llmclient API
        :rtype: str
        """
        # pylint: disable=C0415
        from smarter.apps.llmclient.api.v1.urls import LLMClientApiV1ReverseViews

        path = reverse(
            f"{LLMClientApiV1ReverseViews.namespace}:{LLMClientApiV1ReverseViews.default_llmclient_api_view_by_hashed_id}",
        ...
        if not isinstance(url, str):
            raise SmarterValueError("LLMClient.url_llmclient is not a valid string")
    ...
    def url_chat_config(self) -> str:
        ...
        to retrieve the UI configuration for the llmclient.

        maps to "<int:llmclient_id>/config/"
        example: "http://localhost:9357/api/v1/llm-clients/5174/config/"

        :returns: URL for llmclient config API
        :rtype: str
        """
        # pylint: disable=C0415
        from smarter.apps.llmclient.api.v1.urls import LLMClientApiV1ReverseViews
    ...
    def is_authentication_required(self) -> bool:
        ...
        """
        # pylint: disable=C0415
        from smarter.apps.llmclient.models.llmclient_api_key import LLMClientAPIKey

        llmclientapikeys = LLMClientAPIKey.get_cached_objects(llmclient=self)
    ...
    def get_cached_object(
        ...
        **kwargs,
        ...
        .. code-block:: python
            ...
            llmclient = LLMClient.get_cached_object(pk=1)

            # Retrieve a LLMClient instance by name and user profile with caching
            llmclient = LLMClient.get_cached_object(name="example", user_profile=my_user_profile)
    ...
    def get_cached_objects(
        ...
        .. code-block:: python
            ...
            llmclients = LLMClient.get_cached_objects(my_user_profile, invalidate=True)
        ...
        @cache_results()
        def _get_llmclients_for_user_profile_id(
            ...
            logger.debug(
                "%s._get_llmclients_for_user_profile_id() fetched and cached %s llmclients for user_profile_id: %s",
                logger_prefix,
                len(retval),
                user_profile_id,
            )
        ...
        if invalidate and user_profile:
            _get_llmclients_for_user_profile_id.invalidate(user_profile_id=user_profile.id, class_name=cls.__name__)  # type: ignore[union-attr]

        if user_profile:
            return _get_llmclients_for_user_profile_id(user_profile_id=user_profile.id, class_name=cls.__name__)  # type: ignore[return-value]
    ...
    def save(self, *args, asynchronous=False, **kwargs):
        ...
        if should_deploy:
            logger.debug(
                "%s.LLMClient.save() sending llmclient_deploy signal for LLMClient id: %s",
                self.formatted_class_name,
                self.pk,
            )
            llmclient_deploy.send(sender=self.__class__, llmclient=self)
        if should_undeploy:
            logger.debug(
                "%s.LLMClient.save() sending llmclient_undeploy signal for LLMClient id: %s",
                self.formatted_class_name,
                self.pk,
            )
            llmclient_undeploy.send(sender=self.__class__, llmclient=self)
    ...
    def clone(
        ...
        cloned_llmclient = super().clone(new_name=new_name, new_version=new_version, user_profile=user_profile)  # type: ignore
        pk = cloned_llmclient.pk

        try:
            from .llmclient_functions import LLMClientFunctions

            new_llmclient_functions = LLMClientFunctions.objects.get(llmclient_id=pk)
            logger.debug(
                "%s.clone() found LLMClientFunctions for LLMClient id: %s. Cloning related LLMClientFunctions.",
                logger_prefix,
                pk,
            )
            new_llmclient_functions.pk = pk
            new_llmclient_functions.save()
        ...
        try:
            ...
            new_llmclient_plugin = LLMClientPlugin.objects.get(llmclient_id=pk)
            logger.debug(
                "%s.clone() found LLMClientPlugin for LLMClient id: %s. Cloning related LLMClientPlugin.",
                logger_prefix,
                pk,
            )
            new_llmclient_plugin.pk = pk
            new_llmclient_plugin.save()
        ...
        logger.debug("%s.clone() completed Returning cloned instance %s", logger_prefix, cloned_llmclient)
        return cloned_llmclient  # type: ignore[return-value]
