...
class LLMClientCustomDomainDNS(TimestampedModel):
    ...
    verification of an llmclient's custom domain.

    The primary use case for this model is to facilitate the automation and tracking of DNS
    configurations necessary for deploying llmclients on customer-branded domains. By maintaining a
    record of all DNS entries related to an llmclient's custom domain, the platform can automate DNS
    ...
    **Usage Scenarios**
    ...
    - When deploying an llmclient to a custom domain, instances of this model are created to represent
    ...
    - Used internally by deployment and verification processes to track the status and configuration
      of DNS records for each llmclient custom domain.
    ...
    **Example**
    ...
    .. code-block:: python
        ...
        # Create a new DNS record for an llmclient custom domain
        dns_record = LLMClientCustomDomainDNS.objects.create(
            custom_domain=my_custom_domain,
            record_name="_acme-challenge.llmclient.example.com",
            record_type="TXT",
    ...
    #: The name of the DNS record (e.g., "_acme-challenge.llmclient.example.com").
    #: Example: "_acme-challenge.llmclient.example.com"
