...
"""LLMClient api/v1/llmclients base view, for invoking a LLMClient."""
...
class LLMClientApiBaseViewSet(SmarterAuthenticatedNeverCachedWebView):
    ...
    This class serves as the foundational viewset for all llmclient-related APIs in the Smarter platform,
    ...
    **Key Responsibilities:**
    ...
    - **Plugin Discovery and Extensibility:**
      Discovers and initializes plugins for llmclient extensibility, supporting the Smarter LLM Tool Call Plugin architecture.
    ...
    **Django Integration:**
    ...
    - Overrides and extends methods such as ``setup()``, ``dispatch()``, ``get()``, and ``post()`` to provide llmclient-specific logic.
    ...
    **Prompt Completion & LLM Tool Call Plugins:**
    ...
    - Plugins can be discovered and invoked as part of the llmclient's response generation, enabling extensible and dynamic tool use.
    ...
    **Notes:**
    ...
    - Intended to be subclassed by concrete llmclient API views.
    ...
    **See Also:**
        ...
        - LLMClientHelper and PromptHelper for llmclient and prompt session logic.
        - Smarter LLM Tool Call Plugin architecture documentation.
    """

    _llmclient_id: Optional[int] = None
    ...
    @property
    def llmclient_id(self):
        """
        Returns the llmclient ID.

        :return: The llmclient ID.
        :rtype: Optional[int]
        """
    ...
    def chat_helper(self) -> PromptHelper:
        ...
        if self.session_key or self.llmclient:
            self._chat_helper = PromptHelper(
                request=self.smarter_request, session_key=self.session_key, llmclient=self.llmclient
            )
            if self._chat_helper:
                self.helper_logger(
                    f"{self.formatted_class_name} initialized with prompt: {self.chat_helper.prompt}, llmclient: {self.llmclient}"
                )
        else:
            raise SmarterLLMClientException(
                f"PromptHelper not found. request={self.smarter_request} name={self.name}, llmclient_id={self.llmclient_id}, session_key={self.session_key}, user_profile={self.user_profile}"
            )
    ...
    def llmclient_helper(self) -> Optional[LLMClientHelper]:
        ...
        # ensure that we have some combination of properties that can identify an llmclient
        if not (self.url or self.llmclient_id or (self.user_profile and self.name)):
            return None
        try:
        ...
        # smarter.apps.llmclient.models.LLMClient.DoesNotExist: LLMClient matching query does not exist.
        except LLMClient.DoesNotExist as e:
            raise LLMClient.DoesNotExist(
                f"LLMClient not found. request={self.smarter_request} name={self.name}, llmclient_id={self.llmclient_id}, session_key={self.session_key}, user_profile={self.user_profile}"
            ) from e

        self._llmclient_id = self._llmclient_helper.llmclient_id
        if self._llmclient_id:
            logger.debug(
                "%s: %s initialized LLMClientHelper with id: %s, url: %s",
                self.formatted_class_name,
                self._llmclient_helper,
                self._llmclient_id,
                self._url,
            )
        if self._llmclient_helper:
            ...
            self._url = urlparse(self._llmclient_helper.url)  # type: ignore
            self._user = self._llmclient_helper.user
            self._account = self._llmclient_helper.account
            self._user_profile = self._llmclient_helper.user_profile
        logger.debug(
    ...
    def name(self):
        ...
        :return: The name of the llmclient.
        :rtype: Optional[str]
        """
        if self._name:
            return self._name
        self._name = self._llmclient_helper.name if self._llmclient_helper else None
    ...
    def llmclient(self):
        ...
        """
        return self.llmclient_helper.llmclient if self.llmclient_helper else None
    ...
    def url(self) -> Optional[ParseResult]:
        """
        Returns the URL of the llmclient.

        :return: The URL of the llmclient.
    ...
    def setup(self, request: ASGIRequest, *args, **kwargs):
        ...
        - Prepare and set up the :class:`LLMClientHelper` and :class:`PromptHelper` instances, which are used
          throughout the request lifecycle for llmclient-specific logic and prompt session management.
    ...
    def dispatch(self, request: ASGIRequest, *args, name: Optional[str] = None, **kwargs):
        ...
        - Extracts and sets the llmclient ID from the URL parameters, if present.
        ...
        - Handles error conditions such as missing or invalid llmclient configuration, returning
          appropriate HTTP error responses.
        - Loads and attaches plugins for the llmclient, if available.
        ...
        *args
        ...
        name : Optional[str]
            The name of the llmclient, if provided as a URL parameter.
        ...
        **kwargs
        ...
        - LLMClientHelper and PromptHelper for llmclient and prompt session logic.
        """
        self._llmclient_id = kwargs.get("llmclient_id")
        if self._llmclient_id:
            kwargs.pop("llmclient_id")
        if self.llmclient and self.llmclient.user_profile:
            if self._user_profile != self.llmclient.user_profile:
        ...
        logger.debug("%s.dispatch() - id=%s", self.formatted_class_name, self.llmclient_id)
        ...
        logger.debug("%s.dispatch() - llmclient=%s", self.formatted_class_name, self.llmclient)
        ...
        if not self.llmclient_helper:
            raise SmarterLLMClientException(
                f"LLMClientHelper not found. request={self.smarter_request} name={self.name}, llmclient_id={self.llmclient_id}, session_key={self.session_key}, user_profile={self.user_profile}"
            )
        if not self.llmclient_helper.ready:
            data = {
                "data": {
                    "error": {
                        ...
                        "name": self.llmclient_helper.name,
                        "url": self.llmclient_helper.url,
                    },
                },
            }
        ...
        if self.llmclient_helper.is_authentication_required and not is_authenticated_request(request):
        ...
        self.plugins = LLMClientPlugin().plugins(llmclient=self.llmclient)
        self.functions = LLMClientFunctions().functions(llmclient=self.llmclient)

        if self.llmclient_helper.is_llmclient:
            logger.debug("%s.dispatch(): account=%s", self.formatted_class_name, self.account)
            logger.debug("%s.dispatch(): llmclient=%s", self.formatted_class_name, self.llmclient)
    ...
    def post(self, request, *args, name: Optional[str] = None, **kwargs):
        ...
        This method processes POST requests to the llmclient API endpoint. It determines which
        ...
        1. **URL with default API domain**
            ...
            - ``customer-support``: The llmclient's name.
            - ``3141-5926-5359``: The llmclient's account number.
        ...
        **kwargs
        ...
        logger.debug(
            "%s.post() - provider=%s", self.formatted_class_name, self.llmclient.provider if self.llmclient else None
        )
        ...
        logger.debug("%s.post() - llmclient: %s", self.formatted_class_name, self.llmclient)
        ...
        handler: SmarterChatHarnessProtocol = smarter_compatible_client.get_smarter_harness(
            request=request, provider_name=self.llmclient.provider
        )
        ...
        if not self.chat_helper.prompt:
            raise SmarterLLMClientException(
                f"Prompt not found. This is a bug. request={self.smarter_request} name={self.name}, llmclient_id={self.llmclient_id}, session_key={self.session_key}, user_profile={self.user_profile}"
            )
        if not self.data:
            raise SmarterLLMClientException(
                f"POST data is empty. request={self.smarter_request} name={self.name}, llmclient_id={self.llmclient_id}, session_key={self.session_key}, user_profile={self.user_profile}"
            )
        # RequestMixin.data is more relaxed than the provider expects, so we validate here
        if not isinstance(self.data, (dict, list)):
            raise SmarterLLMClientException(
                f"POST data is not a dict or list. request={self.smarter_request} name={self.name}, llmclient_id={self.llmclient_id}, session_key={self.session_key}, user_profile={self.user_profile}, data_type={type(self.data)}"
            )
        # likewise, AccountMixin.user can accept AnonymousUser, but providers expect a real User
        if not isinstance(self.user, User):
            raise SmarterLLMClientException(
                f"User is not a valid User instance. request={self.smarter_request} name={self.name}, llmclient_id={self.llmclient_id}, session_key={self.session_key}, user_profile={self.user_profile}, user_type={type(self.user)}"
            )
