...
Functions:
    ...
    - get_cached_llmclients_owned_by_user_profile(user_profile)
    - invalidate_cached_llmclients_owned_by_user_profile(user_profile)
    - get_cached_llmclients_shared_with_user_profile(user_profile)
    - invalidate_cached_llmclients_shared_with_user_profile(user_profile)
    - get_cached_llmclients_available_to_user_profile(user_profile)
    - invalidate_cached_llmclients_available_to_user_profile(user_profile)
    - invalidate_all_cached_llmclients_for_user_profile(user_profile)
...
Dependencies:
    ...
    - smarter.apps.llmclient.models.LLMClient
    - smarter.apps.llmclient.serializers.LLMClientSerializer
"""
...
@cache_results()
def _get_cached_llmclients_owned_by_user_profile(user_profile_id: int) -> models.QuerySet[LLMClient]:
...
def get_cached_llmclients_owned_by_user_profile(user_profile: UserProfile) -> models.QuerySet[LLMClient]:
    """
    ...
    .. code-block:: python
        ...
        >>> llmclients = get_cached_llmclients_owned_by_user_profile(user_profile)
        >>> for bot in llmclients:
        ...     print(bot.name)

    .. seealso::

        - :func:`invalidate_cached_llmclients_owned_by_user_profile` - Invalidate the cache for owned LLMClients of a user profile.
    """

    return _get_cached_llmclients_owned_by_user_profile(user_profile.id)  # type: ignore


def invalidate_cached_llmclients_owned_by_user_profile(user_profile: UserProfile) -> None:
    _get_cached_llmclients_owned_by_user_profile.invalidate(user_profile.id)  # type: ignore


@cache_results()
def _get_cached_llmclients_shared_with_user_profile(user_profile_id: int) -> models.QuerySet[LLMClient]:
    user_profile = UserProfile.objects.get(id=user_profile_id)  # type: ignore
...
def get_cached_llmclients_shared_with_user_profile(user_profile: UserProfile) -> models.QuerySet[LLMClient]:
    """
    ...
    .. code-block:: python
        ...
        >>> user_profile = UserProfile.objects.get(pk=1)
        >>> shared_llmclients = get_cached_llmclients_shared_with_user_profile(user_profile)
        >>> for bot in shared_llmclients:
        ...     print(bot.name)

    .. seealso::

        - :func:`invalidate_cached_llmclients_shared_with_user_profile` - Invalidate the cache for shared LLMClients of a user profile.
    """

    return _get_cached_llmclients_shared_with_user_profile(user_profile.id)  # type: ignore


def invalidate_cached_llmclients_shared_with_user_profile(user_profile: UserProfile) -> None:
    _get_cached_llmclients_shared_with_user_profile.invalidate(user_profile.id)  # type: ignore


@cache_results()
def _get_cached_llmclients_available_to_user_profile(user_profile_id) -> models.QuerySet[LLMClient]:
    user_profile = UserProfile.objects.get(id=user_profile_id)  # type: ignore
...
def get_cached_llmclients_available_to_user_profile(user_profile: UserProfile) -> models.QuerySet[LLMClient]:
    """
    ...
    .. code-block:: python
        ...
        >>> user_profile = UserProfile.objects.get(pk=1)
        >>> available_llmclients = get_cached_llmclients_available_to_user_profile(user_profile)
        >>> for bot in available_llmclients:
        ...     print(bot.name)

    .. seealso::

        - :func:`invalidate_cached_llmclients_available_to_user_profile` - Invalidate the cache for available LLMClients of a user profile.
    """

    return _get_cached_llmclients_available_to_user_profile(user_profile.id)  # type: ignore


def invalidate_cached_llmclients_available_to_user_profile(user_profile: UserProfile) -> None:
    _get_cached_llmclients_available_to_user_profile.invalidate(user_profile.id)  # type: ignore


def invalidate_all_cached_llmclients_for_user_profile(user_profile: UserProfile) -> None:
    """
    ...
    .. seealso::
        ...
        - :func:`invalidate_cached_llmclients_available_to_user_profile` - Invalidate the cache for available LLMClients of a user profile.
    """
    invalidate_cached_llmclients_owned_by_user_profile(user_profile=user_profile)
    invalidate_cached_llmclients_shared_with_user_profile(user_profile=user_profile)
    invalidate_cached_llmclients_available_to_user_profile(user_profile=user_profile)
