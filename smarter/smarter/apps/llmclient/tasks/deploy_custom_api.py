...
def deploy_custom_api(llmclient_id: int):
    """
    ...
    5. Creates a Route53 A record for the llmclient's custom domain.
    6. Verifies the hosted zone of the custom domain.
    7. Sends a post-deploy signal for the llmclient API.
    ...
    domain_name = llmclient.custom_domain

    if not domain_name:
        logger.warning(
            "%s Custom domain is missing or is not yet validated for %s llmclient %s task_id: %s. Nothing to do, returning.",
            prefix,
            llmclient.user_profile.account.company_name,
            llmclient.name,
            task_id,
        )
