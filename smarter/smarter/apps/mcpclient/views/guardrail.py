"""Views for mcpclient-related pages in the Smarter web console."""
    ...
    **Example usage**::
    ...
    def get(self, request: HttpRequest, *args, **kwargs) -> HttpResponse:
        ...
        if not isinstance(self.name, str):
            logger.error("MCPClient name should be type str but received %s. This is a bug.", type(self.name))
            return SmarterHttpResponseNotFound(request=request, error_message="MCPClient name is required")
        self.mcpclient = MCPClient.get_cached_object(user=request.user, name=self.name)  # type: ignore

        if not self.mcpclient:
            logger.error("%s.post() MCPClient %s not found for user %s.", self.formatted_class_name, self.name, request.user.username)  # type: ignore[union-attr]
            return SmarterHttpResponseNotFound(request=request, error_message="MCPClient not found")

        logger.debug(
            "%s.post() Rendering mcpclient detail view for %s of kind %s, kwargs=%s.",
...
class MCPClientListView(SmarterAuthenticatedNeverCachedWebView):
    """
    ...
    template_path = "mcpclient/mcpclient_list.html"
    mcpclients: Sequence[MCPClient]

    @property
    def formatted_class_name(self) -> str:
        """Returns a formatted string of the class name for logging purposes."""
        class_name = f"{__name__}.{MCPClientListView.__name__}[{id(self)}]"
        return self.formatted_text(class_name)

    def get(self, request: ASGIRequest, *args, **kwargs):
        """
        Handle GET requests to render the mcpclient list view.

        This method retrieves all mcpclients available to the authenticated user and renders them in a card-based layout
        """
        ...
        if not isinstance(request.user, User):
            ...
            return SmarterHttpResponseNotFound(request=request, error_message="User is not authenticated")
        self.mcpclients = MCPClient.get_cached_objects(user=request.user)  # type: ignore
        context = {
            "mcpclient_list": {"mcpclients": self.mcpclients, "smarter_admin": smarter_cached_objects.smarter_admin}
        }
