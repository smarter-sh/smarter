...
class TestMCPClientApiBaseViewSet(TestAccountMixin):
    """Test SAM MCPClient Broker."""
    ...
    def setUpClass(cls):
        ...
        super().setUpClass()
        config_path = os.path.join(HERE, "data/mcpclient.yaml")
    ...
    def test_base_class_properties(self):
        base_class = MCPClientApiBaseViewSet(request=self.request)

        # invoke dispatch method in order to set our class properties
        base_class.dispatch(request=self.request, name=self.broker.mcpclient.name)  # type: ignore

        logger.debug("test_base_class_properties() request=%s name=%s", self.request, self.broker.mcpclient.name)  # type: ignore
