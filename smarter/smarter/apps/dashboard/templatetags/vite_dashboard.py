...
- dashboard_vite_assets(entry="index.html"): Django template tag that returns the JS and CSS assets for a given Vite entry.
...
def collect_css(manifest: dict[str, Any], key: str, seen: set[str] | None = None) -> list[str]:
    ...
    logger.debug("%s.collect_css() collected and cached CSS for key=%s: %s", logger_prefix, key, css)
    return css


@register.simple_tag
@cache_results()
def dashboard_vite_assets(entry: str = "index.html") -> dict[str, Any]:
    """
    ...
    logger.debug(
        "%s.dashboard_vite_assets() cached result for entry=%s assets=%s",
        logger_prefix,
        entry,
