...
class TestLLMHostApiBaseViewSet(TestAccountMixin):
    """Test SAM LLMHost Broker."""
    ...
    def setUpClass(cls):
        ...
        super().setUpClass()
        config_path = os.path.join(HERE, "data/llmhost.yaml")
    ...
    def test_base_class_properties(self):
        base_class = LLMHostApiBaseViewSet(request=self.request)

        # invoke dispatch method in order to set our class properties
        base_class.dispatch(request=self.request, name=self.broker.llmhost.name)  # type: ignore

        logger.debug("test_base_class_properties() request=%s name=%s", self.request, self.broker.llmhost.name)  # type: ignore
