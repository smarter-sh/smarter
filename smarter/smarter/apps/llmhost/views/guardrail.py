"""Views for llmhost-related pages in the Smarter web console."""
    ...
    **Example usage**::
    ...
    def get(self, request: HttpRequest, *args, **kwargs) -> HttpResponse:
        ...
        if not isinstance(self.name, str):
            logger.error("LLMHost name should be type str but received %s. This is a bug.", type(self.name))
            return SmarterHttpResponseNotFound(request=request, error_message="LLMHost name is required")
        self.llmhost = LLMHost.get_cached_object(user=request.user, name=self.name)  # type: ignore

        if not self.llmhost:
            logger.error("%s.post() LLMHost %s not found for user %s.", self.formatted_class_name, self.name, request.user.username)  # type: ignore[union-attr]
            return SmarterHttpResponseNotFound(request=request, error_message="LLMHost not found")

        logger.debug(
            "%s.post() Rendering llmhost detail view for %s of kind %s, kwargs=%s.",
...
class LLMHostListView(SmarterAuthenticatedNeverCachedWebView):
    ...
    template_path = "llmhost/llmhost_list.html"
    llmhosts: Sequence[LLMHost]

    @property
    def formatted_class_name(self) -> str:
        """Returns a formatted string of the class name for logging purposes."""
        class_name = f"{__name__}.{LLMHostListView.__name__}[{id(self)}]"
        return self.formatted_text(class_name)

    def get(self, request: ASGIRequest, *args, **kwargs):
        """
        Handle GET requests to render the llmhost list view.

        This method retrieves all llmhosts available to the authenticated user and renders them in a card-based layout
        """
        ...
        if not isinstance(request.user, User):
            ...
            return SmarterHttpResponseNotFound(request=request, error_message="User is not authenticated")
        self.llmhosts = LLMHost.get_cached_objects(user=request.user)  # type: ignore
        context = {"llmhost_list": {"llmhosts": self.llmhosts, "smarter_admin": smarter_cached_objects.smarter_admin}}
