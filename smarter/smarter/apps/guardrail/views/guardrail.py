"""Views for guardrail-related pages in the Smarter web console."""
    ...
    **Example usage**::
    ...
    def get(self, request: HttpRequest, *args, **kwargs) -> HttpResponse:
        ...
        if not isinstance(self.name, str):
            logger.error("Guardrail name should be type str but received %s. This is a bug.", type(self.name))
            return SmarterHttpResponseNotFound(request=request, error_message="Guardrail name is required")
        self.guardrail = Guardrail.get_cached_object(user=request.user, name=self.name)  # type: ignore

        if not self.guardrail:
            logger.error("%s.post() Guardrail %s not found for user %s.", self.formatted_class_name, self.name, request.user.username)  # type: ignore[union-attr]
            return SmarterHttpResponseNotFound(request=request, error_message="Guardrail not found")

        logger.debug(
            "%s.post() Rendering guardrail detail view for %s of kind %s, kwargs=%s.",
...
class GuardrailListView(SmarterAuthenticatedNeverCachedWebView):
    """
    ...
    template_path = "guardrail/guardrail_list.html"
    guardrails: Sequence[Guardrail]

    @property
    def formatted_class_name(self) -> str:
        """Returns a formatted string of the class name for logging purposes."""
        class_name = f"{__name__}.{GuardrailListView.__name__}[{id(self)}]"
        return self.formatted_text(class_name)

    def get(self, request: ASGIRequest, *args, **kwargs):
        """
        Handle GET requests to render the guardrail list view.

        This method retrieves all guardrails available to the authenticated user and renders them in a card-based layout
        """
        ...
        if not isinstance(request.user, User):
            ...
            return SmarterHttpResponseNotFound(request=request, error_message="User is not authenticated")
        self.guardrails = Guardrail.get_cached_objects(user=request.user)  # type: ignore
        context = {
            "guardrail_list": {"guardrails": self.guardrails, "smarter_admin": smarter_cached_objects.smarter_admin}
        }
