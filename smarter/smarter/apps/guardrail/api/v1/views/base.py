...
base_logger = logging.getSmarterLogger(__name__, any_switches=[SmarterWaffleSwitches.GUARDRAIL_LOGGING])
logger = base_logger


# pylint: disable=too-many-instance-attributes
@method_decorator(csrf_exempt, name="dispatch")
class GuardrailApiBaseViewSet(SmarterAuthenticatedNeverCachedWebView):
    """
    Base viewset for all Guardrail API endpoints.

    This class serves as the foundational viewset for all guardrail-related APIs in the Smarter platform,
    ...
    **Key Responsibilities:**
    ...
    - **Lifecycle Management:**
      Handles initialization of Account, Guardrail, GuardrailHelper, and PromptHelper objects, and manages request dispatching
    ...
    **See Also:**
        ...
        - GuardrailHelper and PromptHelper for guardrail and prompt session logic.
        - Smarter LLM Tool Call Plugin architecture documentation.
    """

    _guardrail_id: Optional[int] = None
    ...
    def guardrail_id(self):
        ...
        :rtype: Optional[int]
        """
        return self.guardrail.id if self.guardrail else self._guardrail_id  # type: ignore
    ...
    def name(self):
        ...
        if self._name:
            return self._name
        self._name = self.guardrail.name if self.guardrail else None
    ...
    def formatted_class_name(self) -> str:
        ...
        :rtype: str
        """
        class_name = f"{__name__}.{GuardrailApiBaseViewSet.__name__}[{id(self)}]"
    ...
    def setup(self, request: ASGIRequest, *args, **kwargs):
        ...
        - Prepare and set up the :class:`GuardrailHelper` and :class:`PromptHelper` instances, which are used
          throughout the request lifecycle for guardrail-specific logic and prompt session management.
    ...
    def dispatch(self, request: ASGIRequest, *args, name: Optional[str] = None, **kwargs):
        ...
        initializing the GuardrailHelper and PromptHelper instances, setting up the request context,
        ...
        - Handles error conditions such as missing or invalid guardrail configuration, returning
        ...
        *args
        ...
        name : Optional[str]
            The name of the guardrail, if provided as a URL parameter.
        ...
        **kwargs
        ...
        - GuardrailHelper and PromptHelper for guardrail and prompt session logic.
        """
        self._guardrail_id = kwargs.get("guardrail_id")
        if self._guardrail_id:
            kwargs.pop("guardrail_id")
        else:
            self._name = self._name or name
        if not self.guardrail:
            logger.warning(
                "Could not initialize Guardrail url: %s, name: %s, user: %s, account: %s, id: %s",
                self.url,
                self.name,
                self.user,
                self.account,
        ...
        if not self.guardrail:
            raise SmarterGuardrailException(
                f"GuardrailHelper not found. request={self.smarter_request} name={self.name}, guardrail_id={self.guardrail_id}, session_key={self.session_key}, user_profile={self.user_profile}"
            )
        if not self.guardrail.ready:
            data = {
                "data": {
                    "error": {
                        "message": "Could not initialize Guardrail object.",
                        "account": self.account.account_number if self.account else None,
                        "guardrail": GuardrailSerializer(self.guardrail).data if self.guardrail else None,
                        "user": self.user.username if self.user else None,
                        "name": self.guardrail.name,
                    },
                },
            }
            logger.debug("%s.dispatch() - Guardrail not ready", self.formatted_class_name)
            return JsonResponse(data=data, status=HTTPStatus.BAD_REQUEST.value)

        if self.guardrail:
            charge_authorization(self.guardrail.record_locator, self.__class__.__name__)  # type: ignore
            guardrail_called.send(
    ...
    def options(self, request, *args, **kwargs):
        ...
        """
        logger.debug("%s.options(): guardrail=%s", self.formatted_class_name, self.guardrail)
        response = Response()
