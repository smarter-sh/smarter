...
"""
Cache management utilities for Vectorstore objects.

This module provides functions for efficient type-annotated retrieval and
caching of Vectorstore querysets. It includes utilities to:
...
- Invalidate all Vectorstore-related caches for a user profile
...
Dependencies:
    ...
    - smarter.apps.vectorstore.models.Vectorstore
    - smarter.apps.vectorstore.serializers.VectorstoreSerializer
"""
...
from .serializers import VectorstoreSerializer
...
@cache_results()
def _get_cached_proxies_owned_by_user_profile(user_profile_id: int) -> models.QuerySet[VectorstoreMeta]:
    user_profile = UserProfile.objects.get(id=user_profile_id)  # type: ignore
    retval = VectorstoreMeta.objects.owned_by(user_profile.user)  # type: ignore
    logger.debug(
        "%s.post() Fetching and caching Proxies owned by user: %s",
        logger_prefix,
        logging.formatted_json(VectorstoreSerializer(retval, many=True).data),
    )
    return retval


def get_cached_proxies_owned_by_user_profile(user_profile: UserProfile) -> models.QuerySet[VectorstoreMeta]:
    """
    Retrieve the Proxies owned by the given UserProfile, using caching to optimize performance.

    This function returns a queryset of Vectorstore objects that are owned by the specified user profile.
    ...
    :returns: A Django queryset containing the Vectorstore objects owned by the user.
    :rtype: QuerySet[Vectorstore]
...
@cache_results()
def _get_cached_proxies_shared_with_user_profile(user_profile_id: int) -> models.QuerySet[VectorstoreMeta]:
    user_profile = UserProfile.objects.get(id=user_profile_id)  # type: ignore
    retval = VectorstoreMeta.objects.shared_with(user_profile.user)  # type: ignore
    logger.debug(
...
def get_cached_proxies_shared_with_user_profile(user_profile: UserProfile) -> models.QuerySet[VectorstoreMeta]:
    """
    Retrieve the Proxies shared with the given UserProfile, using caching to optimize performance.

    This function returns a queryset of Vectorstore objects that are shared with the specified user profile.
    The results are cached to reduce database queries and improve performance. If the cache is invalidated,
    the queryset is fetched from the database again and re-cached.

    :param user_profile: The user profile whose shared Proxies should be retrieved.
    :type user_profile: UserProfile
    :returns: A Django queryset containing the Vectorstore objects shared with the user.
    :rtype: QuerySet[Vectorstore]
...
@cache_results()
def _get_cached_proxies_available_to_user_profile(user_profile_id) -> models.QuerySet[VectorstoreMeta]:
    user_profile = UserProfile.objects.get(id=user_profile_id)  # type: ignore
    retval = VectorstoreMeta.objects.with_read_permission_for(user_profile.user)  # type: ignore
    logger.debug(
...
def get_cached_proxies_available_to_user_profile(user_profile: UserProfile) -> models.QuerySet[VectorstoreMeta]:
    """
    Retrieve the Proxies available to the given UserProfile, using caching to optimize performance.

    This function returns a queryset of Vectorstore objects that are available to the specified user profile,
    ...
    :type user_profile: UserProfile
    :returns: A Django queryset containing the Vectorstore objects available to the user.
    :rtype: QuerySet[Vectorstore]
...
def invalidate_all_cached_proxies_for_user_profile(user_profile: UserProfile) -> None:
    """
    Invalidate all cached Vectorstore querysets related to the given UserProfile.

    This function invalidates the caches for all Vectorstore querysets that are related to the specified user profile,
    ...
    :param user_profile: The user profile for which to invalidate cached Vectorstore querysets.
    :type user_profile: UserProfile
    :returns: None
    :rtype: None
