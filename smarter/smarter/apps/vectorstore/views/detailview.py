...
class VectorstoreDetailView(DocsBaseView):
    """
    ...
    .. seealso::
        ...
        :class:`Vectorstore` for vectorstore metadata retrieval.
    ...
    **Example usage**::
    ...
    vectorstore: Optional[VectorstoreMeta] = None
    ...
    def get(self, request, *args, **kwargs) -> HttpResponse:
        ...
        pk_id = VectorstoreMeta.id_from_hashed_id(hashed_id) if hashed_id else None
        if not pk_id:
        ...
        try:
            self.vectorstore = VectorstoreMeta.objects.get(id=pk_id, user_profile=self.user_profile)
            logger.debug(
                "%s.get() Found vectorstore with id %s for user %s.",
            ...
            try:
                if self.user_profile:
                    ...
                    self.vectorstore = VectorstoreMeta.objects.get(id=pk_id, user_profile=admin_user_profile)
                    logger.debug(
                        "%s.get() Found vectorstore with id %s for admin user %s.",
                        self.formatted_class_name,
                        pk_id,
                ...
                try:
                    self.vectorstore = VectorstoreMeta.objects.get(
                        id=pk_id, user_profile=smarter_cached_objects.smarter_admin_user_profile
                    )
                    logger.debug(
                        "%s.get() Found vectorstore with id %s for smarter admin user %s.",
                        self.formatted_class_name,
                        pk_id,
        ...
        if not self.vectorstore:
            logger.error(
                "%s.get() - Vectorstore with id %s not found for user %s or admin users.",
                self.formatted_class_name,
                pk_id,
                self.user_profile.user.username if self.user_profile else "unknown user",
            )
        ...
        logger.debug(
            ...
            self.formatted_class_name,
            self.vectorstore.name if self.vectorstore else "unknown vectorstore",
            kwargs,
        )
        kwargs.pop("name", None)
        kwargs["name"] = self.vectorstore.name if self.vectorstore else "unknown vectorstore"
        ...
        context = {
            ...
            "page_title": self.vectorstore.name if self.vectorstore else "unknown vectorstore",
        }
