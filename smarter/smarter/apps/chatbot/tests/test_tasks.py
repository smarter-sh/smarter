...
class TestChatBotTasks(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.domain_name = "123456789.smarter.sh"
        self.hosted_zone = aws_helper.get_hosted_zone(domain_name=self.domain_name)
        if self.hosted_zone:
            aws_helper.delete_hosted_zone(domain_name=self.domain_name)
    ...
    def test_create_hosted_zone(self):
        self.hosted_zone = aws_helper.get_hosted_zone_id_for_domain(domain_name=self.domain_name)
        self.assertIsNotNone(self.hosted_zone)

    def test_create_custom_domain(self):
        """Test that we can create a custom domain."""

        create_custom_domain(account_id=self.account.id, domain_name=self.domain_name)

        aws_helper.get_hosted_zone_id_for_domain(domain_name=self.domain_name)
        hosted_zone = aws_helper.get_hosted_zone(domain_name=self.domain_name)
    ...
    def test_create_custom_domain_dns_record(self):
        ...
        hosted_zone = aws_helper.get_hosted_zone_id_for_domain(domain_name=self.domain_name)
        self.assertIsNotNone(hosted_zone)
        ...
        dns_record = aws_helper.get_dns_record(
    ...
    def test_verify_custom_domain(self):
        ...
        hosted_zone_id = aws_helper.get_hosted_zone_id_for_domain(domain_name=smarter_settings.root_domain)
    ...
    def test_create_domain_A_record(self):
        ...
        hosted_zone_id = aws_helper.get_hosted_zone_id_for_domain(domain_name=self.domain_name)
    ...
    def test_deploy_default_api(self):
        ...
        hosted_zone_id = aws_helper.get_hosted_zone_id_for_domain(domain_name=smarter_settings.customer_api_domain)
        a_record = aws_helper.get_dns_record(
