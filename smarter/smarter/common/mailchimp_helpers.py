...
def add_list_member(email_address):
    ...
    try:
        ...
        mailchimp_client.set_config({"api_key": smarter_settings.mailchimp_api_key, "server": MAILCHIMP_SERVER})
        mailchimp_api_response = mailchimp_client.ping.get()
        logger.info("Connected to MailChimp API: %s", mailchimp_api_response)

        mailchimp_api_response = mailchimp_client.lists.add_list_member(
