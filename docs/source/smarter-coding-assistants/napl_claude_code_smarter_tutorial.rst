.. ============================================================
   Northern Aurora Power & Light — IT Department
   Getting Started: Claude Code with the Smarter Platform
   ============================================================

.. _napl-claude-code-smarter:

======================================================
Getting Started with Claude Code on the Smarter Platform
======================================================

| **Document owner:** IT Department — Custom Programming Area
| **Platform:** Smarter (on-premise LLM gateway)
| **Coding assistant:** Claude Code (Anthropic)
| **Audience:** NAPL Programmers
...
We will use **Claude Code** with the **NAPL Smarter platform** to establish
a fully functional, enterprise-governed AI coding assistant that routes all
LLM traffic through NAPL's centrally managed on-premise Smarter gateway.
...
* Confirmed the end-to-end path by asking Claude Code to explain a short
  Python function, with all traffic flowing through the NAPL gateway.
...
.. note::
   ...
   NAPL infrastructure team and is already complete.  Cost-tracking code
   ...
   key, which authenticates *you* to the NAPL gateway.  The gateway then
...
.. list-table:: Required Knowledge
   ...
   * - Smarter account
     - You have already been provisioned a Smarter account and know how to
       log in to the Smarter web console at ``https://smarter.napl.internal``
       (internal URL — VPN required).
   * - Network access
     - Your workstation can reach the internal NAPL network (directly or
       via VPN).
...
If the NAPL artifact repository mirrors the Smarter CLI releases, download
...
your Smarter username, password, and the NAPL internal Smarter API base URL:
...
* **Username / password:** your NAPL SSO or Smarter account credentials.
* **Environment / API base URL:** ``https://smarter.napl.internal``
...
.. code-block:: yaml
   ...
   metadata:
     ...
     description: >
       ...
       and Claude Code to authenticate to the NAPL Smarter gateway.
   spec: {}
...
NAPL Smarter gateway.  Two environment variables control this:
...
.. code-block:: json
   ...
   {
     "env": {
       "ANTHROPIC_BASE_URL": "https://smarter.napl.internal",
...
providers (such as Anthropic).  Think of it as NAPL's air-traffic control
...
default it calls ``https://api.anthropic.com`` directly.  For NAPL we
...
.. list-table:: Claude Code Gateway Environment Variables
   ...
   * - Variable
     - Purpose
   * - ``ANTHROPIC_BASE_URL``
     - Overrides the API base URL; set to the NAPL Smarter gateway URL
   * - ``ANTHROPIC_AUTH_TOKEN``
...
Confirm the CLI is connected to the NAPL gateway and your credentials are
valid:
...
.. code-block:: text
   ...
   environment: prod
   api_base:    https://smarter.napl.internal
...
``https://smarter.napl.internal`` — not ``api.anthropic.com``:

.. code-block:: text

   API endpoint : https://smarter.napl.internal
...
Verify that Claude Code is using the NAPL-approved model.  At the Claude
Code prompt:
...
* Claude Code is routing through the NAPL Smarter gateway.
...
#. Re-run ``smarter configure`` and re-enter your NAPL Smarter username,
   password, and the correct base URL (``https://smarter.napl.internal``).
#. If you have changed your NAPL password recently, make sure you use the
   updated password.
...
.. list-table::
   ...
   * - VPN is not connected; the gateway hostname cannot be resolved
     - Connect to the NAPL VPN and retry
...
* If the model you need is not available, contact the IT Department to
  request it be enabled on the NAPL Smarter gateway.
* Do not attempt to access Anthropic models outside the list of
  NAPL-approved models — direct calls to ``api.anthropic.com`` bypass
  all NAPL governance controls and are not permitted per NAPL IT policy.
...
resolve the issue, contact the NAPL IT helpdesk and include:
...
.. list-table:: NAPL IT Contact Information
   :widths: 40 60
   :header-rows: 1
   ...
   * - Channel
     - Address / Link
   * - Helpdesk email
     - ``helpdesk@napl.internal``
   * - IT ServiceNow portal
     - ``https://itsm.napl.internal``
   * - Slack channel
     - ``#it-ai-tooling``
...
.. list-table:: NAPL Internal Resources (VPN required)
   :widths: 45 55
   :header-rows: 1
   ...
   * - IT ServiceNow portal
     - ``https://itsm.napl.internal``
   * - Internal Slack: AI tooling support
     - ``#it-ai-tooling``

----

*Northern Aurora Power & Light — IT Department*
*Tutorial last reviewed: March 2026*
*Document maintained by: IT Custom Programming Area*
*For corrections or additions, open a ticket in ServiceNow or post in* ``#it-ai-tooling``
