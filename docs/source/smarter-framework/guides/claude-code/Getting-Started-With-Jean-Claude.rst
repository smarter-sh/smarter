...
Claude-backed llmclient — all in under fifteen minutes.
...
Smarter manages every resource — providers, llmclients, plugins — through
...
prompt, and optional data plugins. End users interact with llmclients, not
providers directly.
...
List available llmclients to find one configured with an Anthropic provider:
...
for use in llmclients and the Workbench.

.. tip::

   With both providers registered, create two llmclients — one backed by Opus,
