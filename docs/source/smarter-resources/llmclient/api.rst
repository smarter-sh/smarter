...
- ``smarter.apps.llmclient.models.LLMClientHelper``: Maps a url to its
...
mapping individual llmclients to DNS subdomain records aliased to the
...
- ``'api.example.com == llmclient.custom_domain'`` is a LLMClientCustomDomain
  object
...
``smarter.apps.llmclient.middleware.security.SmarterSecurityMiddleware`` in order
...
``smarter.apps.llmclient.middleware.cors.SmarterCorsMiddleware`` in order to
...
``smarter.apps.llmclient.middleware.csrf.SmarterCsrfViewMiddleware`` in order to
...
per-customer and per-llmclient certificates and the requisite DNS TXT
challenge records.
