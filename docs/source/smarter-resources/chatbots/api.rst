...
ChatBots can also run in pre-production ‘sandbox’ mode from inside the
Smarter web console.

Management
----------

There are Django ``manage.py`` commands for the complete ChatBot
lifecycle, namely, for deploying the ChatBot so that it can begin
...
manage all aspects of a ChatBot. Meanwhile, there’s Django Admin to
...
- Default ChatBot domain names:
  [subdomain].[####-####-####].[environment].smarter.sh/chatbot/
...
ChatBot is not trivial. Note the following code resources for working
with chatbot urls:

- ``smarter.apps.chatbot.models.ChatBotHelper``: Maps a url to its
  ChatBot, Plugin list, Account and User objects.
...
mapping individual chatbots to DNS subdomain records aliased to the
...
- ``ChatBotCustomDomain.is_verified == True``. An asynchronous task
  verifies the domain NS records.
- ``/chatbot/`` is the same URL endpoint used by default domains.

When using a custom domain,
``ChatBot.hostname == ChatBot.custom_domain`` once the following
